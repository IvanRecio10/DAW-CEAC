<?php 
	...
	$sizeMax = 90000; //bytes
 	...
 	if (in_array($ext, $extPermitidas) && in_array($type, $typePermitidos) && $error == 0 && $sizeMax <= $sizeMax) {
 		...
 		//Funcion file_exists(ruta/nameFichero) = devuleve True or False si encuentra el fichero en la ruta

 		if (file_exists("upload/" . $name)) {
 			$codigo = 1;
 		}else{
 			//Funcion move_uploades(tmp_name, rutaDestino/nameFichero) = devuelve T/F si se ha subido correctamente el fichero a la ruta del destino
